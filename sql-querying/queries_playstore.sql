...
SELECT category, COUNT(*) as number_apps_in FROM analytics GROUP BY category;
...
SELECT SUM(reviews) AS total_num_of_reviews from analytics;

--14 Find all the categories that have more than 300 apps in them.
SELECT category, COUNT(category) as num_apps FROM analytics GROUP BY category HAVING COUNT(category) > 300;
